<!-- FOOTER -->
<footer class="d-flex flex-column flex-md-row flex-wrap justify-content-between justify-content-sm-center align-items-center py-3 my-4 mt-auto border-top">
    <div class="col-md-4 d-flex align-items-center mb-3 mb-md-0">
        <a href="/" class="me-2 text-muted text-decoration-none lh-1">
            <svg class="bi" width="30" height="24"><use xlink:href="#bootstrap"></use></svg>
        </a>
        <span class="text-muted">© 2024 Made by <a href="">Alberto Perez</a></span>
    </div>
    <span class="text-muted text-center mb-3 mb-md-0"> This items & prices correspond with World of Warcraft Cataclysm classic - Gehennas EU</span>  

    <ul class="nav col-md-4 justify-content-lg-end justify-content-md-center list-unstyled d-flex me-2">
        <li class="ms-3"><a class="text-muted" href="#"><i class="bi bi-linkedin" style="font-size: 24px;"></i></a></li>
        <li class="ms-3"><a class="text-muted" href="#"><i class="bi bi-github" style="font-size: 24px;"></i></a></li>
    </ul>
</footer>

 <!-- Cargar jQuery primero -->
 <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.1/jquery.min.js" integrity="sha512-v2CJ7UaYy4JwqLDIrZUI/4hqeoQieOmAZNXBeQyjo21dadnwR+8ZaIJVT8EE2iyI61OV8e6M8PP2/4hpQINQ/g==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <!-- Luego jQuery UI -->
    <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.min.js"></script>
    <!-- Después Bootstrap -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
    <!-- Chart.js -->
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <!-- Chosen jQuery plugin -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/chosen/1.8.7/chosen.jquery.min.js"></script>
    <!-- Ignite UI -->
    <script src="https://cdn-na.infragistics.com/igniteui/2023.2/latest/js/infragistics.core.js"></script>
    <script src="https://cdn-na.infragistics.com/igniteui/2023.2/latest/js/infragistics.lob.js"></script>
    <!-- DataTables -->
    <script type="text/javascript" charset="utf8" src="https://cdn.datatables.net/1.10.25/js/jquery.dataTables.js"></script>
   


