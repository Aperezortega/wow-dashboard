...
function getSales($conn, $id_item){
    $sql = "SELECT SUM(quantity) as sold FROM transactions WHERE id_item = $id_item AND type = 'seller'";
    $result = $conn->query($sql);
    $sold = $result->fetch_assoc()['sold'];
    return $sold;
}
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            $item = array(
                ...
                'profit' => $profit,
                'sold' => getSales($conn, $row['id_item']) ?? 0,
            );
            $items[] = $item;
        }
    }
